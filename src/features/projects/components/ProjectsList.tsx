...
interface ProjectsListProps {
  ...
  entrepreneurs: { id: string; profile: { full_name: string } }[]
  userRole: UserRole
}
...
export function ProjectsList({ projects, entrepreneurs, userRole }: ProjectsListProps) {
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter(project => {
      if (selectedEntrepreneur !== 'all' && project.entrepreneur_id !== selectedEntrepreneur) return false
      if (selectedStatus !== 'all' && project.status !== selectedStatus) return false
      if (searchTerm && !project.title.toLowerCase().includes(searchTerm.toLowerCase())) return false
      return true
    })
  }, [projects, selectedEntrepreneur, selectedStatus, searchTerm])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Proyectos y Metas</h1>
          <p className="text-foreground-secondary mt-1">
            Gestiona tus metas y progreso en la academia
          </p>
        </div>
        {userRole === 'admin' && (
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-4">
          ...
          {/* Entrepreneur filter (admin only) */}
          {userRole === 'admin' && entrepreneurs.length > 0 && (
            <select
            ...
            >
              <option value="all">Todos los empresarios</option>
              {entrepreneurs.map(l => (
      ...
      {filteredProjects.length > 0 ? (
        ...
        <Card className="p-12 text-center">
          ...
          <p className="text-foreground-secondary mb-6">
            {searchTerm || selectedStatus !== 'all' || selectedEntrepreneur !== 'all'
              ? 'No se encontraron proyectos con los filtros seleccionados'
              : 'Comienza creando tu primer proyecto o meta personal'}
          </p>
          {userRole === 'admin' && (
            <Link href="/projects/new">
              <Button>
  ...
  return (
    <Card className="p-5 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 mb-4">
        ...
        {userRole === 'admin' && project.entrepreneur && (
          <div className="flex items-center gap-2 text-sm">
            <BriefcaseIcon className="w-4 h-4 text-foreground-secondary" />
            <span className="text-foreground-secondary truncate">
              {project.entrepreneur.profile?.full_name || 'Empresario'}
            </span>
          </div>
        )}
